...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	lotteryChanceRepository := repository.NewLotteryChanceRepository(client)
	lotteryRecordRepository := repository.NewLotteryRecordRepository(client)
	lotteryDrawLimiter := repository.NewLotteryDrawLimiter(redisClient)
	lotteryService := service.NewLotteryService(client, userRepository, lotteryChanceRepository, lotteryRecordRepository, settingRepository, billingCacheService, lotteryDrawLimiter)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, lotteryService)
	userService := service.NewUserService(userRepository, settingRepository, apiKeyAuthCacheInvalidator, billingCache)
	redeemCache := repository.NewRedeemCache(redisClient)
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, lotteryService)
	...
	consumptionRankingHandler := handler.NewConsumptionRankingHandler(consumptionRankingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, lotteryHandler, consumptionRankingHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
